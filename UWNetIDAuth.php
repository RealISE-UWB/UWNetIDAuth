...
$wgExtensionCredits['other'][] = array(
        ...
        'version' => '0.1',
        'path' => __FILE__,
        'author' => array( 'Finn Herzfeld' ),
        'url' => 'https://students.washington.edu/finnian/mwauth',
);
 
if (empty($wgDefaultUserOptions['authURL']))
        die('Please set up the auth URL first : $wgDefaultUserOptions[\'authURL\'] = "https://students.washington.edu/finnian/mwauth"');
 
 
// Now for the code:
 
$wgHooks['UserLoadFromSession'][] = 'fnUWNetIDAuthenticateHook';
 ...
 */
function fnUWNetIDAuthenticateHook($user, &$result)
{
        ...
        if (isset($_REQUEST["title"]))
        {
                ...
                if ($_REQUEST["title"] == $lg->specialPage("Userlogin"))
                {
                        ...
                        // Get username
                        $response = getUWNetID($_REQUEST['token']);
                        
                        // Get MediaWiki user
                        $u = User::newFromName($response->username);
...
function getUWNetID($token) {
    global $wgDefaultUserOptions;
    //if(isset($_SESSION['UWNetIDLoggedInUser'])) {
    //    return $_SESSION['UWNetIDLoggedInUser'];
    //}
    if(!isset($token)) {
        header("Location: ".$wgDefaultUserOptions['authURL']."/auth");
        exit();
    } else {
        $result = json_decode(file_get_contents($wgDefaultUserOptions['authURL']."/fetch.php?token=".urlencode($token)));
        if($result->result == "success") {
            $_SESSION['UWNetIDLoggedInUser'] = $result->username;
            return $result;
        } else {
            error_log("Login error ".$result->error->code.": ".$result->error->msg);
        }
    }
}
